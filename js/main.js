...
function themvaogiohang(productid) {
  ...
  let user1 = ktra_user(); // Lấy thông tin người dùng từ localStorage
  if (!user1) {
    chua_dangnhap();
    return;
  }
  ...
  if (existingProduct) {
    ...
    product1.quantity = 1; // Đặt số lượng ban đầu là 1
    user_cart.push(product1); // Thêm sản phẩm vào giỏ hàng
  }
