...
document.getElementById('account-log-in').addEventListener('submit', function (event) {
  ...
  let user = users.find(user => user.tel === String(log_in_tel));
  let AD0 = JSON.parse(localStorage.getItem('ADMIN0')) || {};

  if (user && user.status && user.tel !== String(AD0.tel)) {
    if (user.password === String(log_in_password)) {
