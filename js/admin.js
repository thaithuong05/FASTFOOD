...
document.addEventListener('DOMContentLoaded', function () {
  let adminData = JSON.parse(localStorage.getItem('ADMIN1')) || {}; // Lấy thông tin admin từ localStorage

  if (adminData && adminData.admin_username) {
    // Nếu có thông tin đăng nhập, hiển thị giao diện admin
    display_account_admin.style.display = 'none';
    open_admin.style.display = 'flex';
    name_admin.innerHTML = adminData.admin_username; // Hiển thị tên admin
    displayUsers();
  } else {
...
function displayUsers() {
  ...
  ql_user_list.innerHTML = ''; // Xóa nội dung cũ

  if (users.length === 0) {
    ql_user_list.innerHTML = 'Chưa có tài khoản người dùng !';
  } else {
    console.log(users.length); // Kiểm tra số sản phẩm
    users.forEach((users) => {
